              ...
              <div className="flex flex-col md:flex-row mt-5 md:gap-5">
                ...
                <div className="md:w-8/12 w-full flex flex-col flex-wrap h-full gap-5">
                  {["Danger", "Warning", "Success"].map((color) => {
                    const filteredCards = colorCard.filter(
                      (colorcard) => colorcard.rar && colorcard.rar.color === color
                    );

                    return filteredCards.map((colorcard, index) => (
                      <div key={index}> 
                        {colorcard.rar && colorcard.rar.color ? (
                          <CardInfo
                            title={translations["Reach Amount Spent Ratio"]}
                          ...
                          // Handle the case where rar or rar.color is not defined
                  ...
                  })}
                  {
                    ["Danger", "Warning", "Success"].map((color) => {
                      const filteredCards = colorCard.filter(
                        (colorcard) => colorcard.ctr && colorcard.ctr.color === color
                      );

                      return filteredCards.map((colorcard, index) => (
                        <div key={index}>
                          {colorcard.ctr && colorcard.ctr.color ? (
                            <CardInfo
                            ...
                            // Handle the case where ctr or ctr.color is not defined
                            <p>Invalid data structure for colorcard at index {index}</p>
                          )}
                        </div>
                      ));
                ...
                {/* Chart */}
                < div className="hidden md:flex justify-between flex-col w-full mt" >
  ...
  return (
    <main className="bg-slate-100 max-sm:overflow-hidden max-h-full">
     ...
     <ContainerCard>
          ...
          <div className="flex p-4 ml-3 pb-1 items-center sm:ml-2 sm:p-3 md:ml-4 md:p-4">
              {selectedData && (
                <img
                  src={
